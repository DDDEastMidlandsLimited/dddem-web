...
describe('Given the Footer is rendered', () => {
  const companies = { companiesList: gold, selectedCompanyDescription: "", setSelectedCompanyDescription: jest.fn() };
  ...
  test('it should fire an event on click', () => {
    ...
    expect(companies.setSelectedCompanyDescription).toHaveBeenCalled();
    expect(companies.setSelectedCompanyDescription).toHaveBeenCalledTimes(1);
    expect(companies.setSelectedCompanyDescription).toHaveBeenCalledWith("<h2>lol</h2>");
  });
});
